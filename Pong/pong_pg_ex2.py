...
if __name__ == "__main__":
    ...
    agent = PGAgentV2(state_size, len(cmd))
    if FRESH_START:
        with open('./logs/{}_pg-{}.log'.format(env_name, SUFFIX), "wb") as f:
            np.savetxt(f, np.array(["episode", "score", "time"]).reshape(1, -1), fmt='%s', delimiter=",")
    else:
        try:
           agent.load('./models/{}_pg-{}.h5'.format(env_name, SUFFIX))
           print("Model {}_pg-{}.h5 loaded.".format(env_name, SUFFIX))
        except:
    ...
    try:
        while True:
            ...
            if done:
                ...
                with open('./logs/{}_pg-{}.log'.format(env_name, SUFFIX), "ab") as f:
                    np.savetxt(f, np.array([episode, score, time]).reshape(1, -1), fmt="%d", delimiter=",")
                if episode > 1 and episode % 50 == 0:
                    agent.save('./models/pong-v0_pg_{}-{}.h5'.format(episode, SUFFIX))
                episode += 1
                score = 0
                time = 0
                state = env.reset()
        ...
        # catch keyboard interruption and save final model
        agent.save('./models/pong-v0_pg-{}.h5'.format(SUFFIX))
