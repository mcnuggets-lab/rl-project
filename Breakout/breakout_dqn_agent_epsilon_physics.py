...
from modified_dqn import ModifiedDQNAgent
from epsilon_physics import ModifiedLinearAnnealedPolicy, EpsilonPhysicsPolicy
...
class AtariProcessor(Processor):
    def __init__(self):
        ...
        self.frame = None  # save the frame for the physics agent to use
...
policy = ModifiedLinearAnnealedPolicy(EpsilonPhysicsPolicy(), attr='eps_phy', value_max=1., value_min=.1, value_test=.05, nb_steps=1000000)
...
dqn = ModifiedDQNAgent(model=model, nb_actions=nb_actions, policy=policy, memory=memory,
...
if args.mode == 'train':
    ...
    weights_filename = 'wts/dqn_{}_weights_epsphy.h5f'.format(args.env_name)
    checkpoint_weights_filename = 'wts/dqn_' + args.env_name + '_weights_{step}_epsphy.h5f'
    log_filename = 'dqn_{}_log_epsphy.json'.format(args.env_name)
    ...
    weights_filename = 'wts/dqn_Breakout-v0_weights_5500000_epsphy.h5f'.format(args.env_name)
    if args.weights:
    ...
    dqn.load_weights(weights_filename)
    dqn.training = False
    dqn.test_policy = EpsilonPhysicsPolicy(eps_phy=0.01, eps_ran=0.00)  # set a small epsilon for test policy to avoid getting stuck
